...
-  you need a server to get data from it,so open terminal and cd to server file.
write npm start to run your sever.
- cd to eccomerce with cd ..  then write yarn or npm install to install all packages.
- write react-native run-ios (if you are in mac os you need xcode be installed) or react-native run-android (android SDK or android studio shold be installed).
